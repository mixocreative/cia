...
**Claude Code, as a bare skill file｜直接手動放置 Skill 檔案：**
...
## Structure of skills/cia/SKILL.md｜Skill 檔案結構

| Section｜章節分類 | Purpose｜功能用途 |
|---|---|
| 0 | Routing hard rules, trigger vocabulary, runtime discovery, six-step protocol, overrides, autonomy contract, mandatory sweeps｜路由硬性規則、觸發詞彙、執行環境探索、六階段流程、專案覆寫設定、自主執行契約、強制掃描項目 |
| 1 | Fundamental audit doctrine: outcomes over technologies, evidence before accusation, version-aware external facts｜稽核基本教條：重視實際結果而非技術宣稱、先講求證據再進行指控、外部事實需對齊版本 |
| 2 | Viable System Model governance pass (Systems 1–5)｜VSM 治理檢查（System 1–5） |
| 3–6 | Context discovery and context templates｜情境探索與情境範本 |
| 7 | Universal test matrix｜通用測試矩陣 |
| 8 | Domain audit checklist｜領域檢查清單 |
| 9–10 | Finding format and severity model｜發現項目格式與嚴重度評估模型 |
